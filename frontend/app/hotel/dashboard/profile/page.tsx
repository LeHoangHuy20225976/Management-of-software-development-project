/**
 * Hotel Profile (DB-aligned fields only)
 */
...
export default function HotelProfilePage() {
  ...
  const handleSave = async () => {
    try {
      ...
      alert('💾 Đã lưu thông tin khách sạn');
      setIsEditing(false);
    } catch (error) {
      console.error('Error saving hotel info:', error);
      alert('Không thể lưu thông tin, thử lại sau.');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-blue-50 border-blue-100">
        <p className="text-sm text-gray-800">
          Chỉ hiển thị và cập nhật các trường có trong bảng <code>Hotel</code>.
          Các tiện ích/ chính sách cần lấy từ bảng liên quan (Facilities,
          Settings) nên không cấu hình tại đây.
        </p>
      </Card>
    </div>
  );
