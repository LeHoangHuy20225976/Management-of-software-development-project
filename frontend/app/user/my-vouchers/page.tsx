  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild>
          <Link href="/search">🎟️ Tìm thêm mã</Link>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <Card>
          <div className="text-center">
            ...
            <div className="text-3xl font-bold text-[#0071c2]">{coupons.length}</div>
      ...
      <Card>
        <div className="flex flex-wrap gap-3">
          ...
          >
            Còn hạn ({coupons.filter(c => new Date(c.expiryDate) >= today).length})
          </Button>
          <Button
          ...
          >
            Hết hạn ({coupons.filter(c => new Date(c.expiryDate) < today).length})
          </Button>
        </div>
      </Card>
      ...
      {filteredCoupons.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <p className="text-gray-700 font-medium mb-4">Bạn chưa có mã giảm giá nào.</p>
          <Button asChild>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredCoupons.map((coupon) => {
            ...
            return (
              <Card key={coupon.id} className={cn("flex flex-col justify-between", isExpired && "opacity-60 bg-gray-50")}>
                <div>
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                    Áp dụng tại: <span className="font-semibold text-gray-800">{coupon.hotelName}</span>
                  </p>
                  <div className="mb-4">
                ...
                <div className="text-sm text-center font-medium text-gray-600 border-t pt-3 mt-4">
                  ...
                  <span className={cn(isExpired ? "text-red-600" : "text-green-600", "font-bold")}>
