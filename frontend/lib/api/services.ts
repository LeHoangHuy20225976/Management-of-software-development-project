...
export const bookingsApi = {
  ...
  async update(id: string, bookingData: Partial<Booking>): Promise<Booking> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      const bookings = getMockBookings();
      return bookings.find(b => b.booking_id === Number(id)) || bookingData as Booking;
    }
  ...
  async calculatePrice(data: { room_id: number; check_in_date: string; check_out_date: string; people: number }): Promise<{ total_price: number }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      // Mock calculation
      const nights = Math.ceil((new Date(data.check_out_date).getTime() - new Date(data.check_in_date).getTime()) / (1000 * 60 * 60 * 24));
...
export const hotelManagerApi = {
  ...
  async updateBookingStatus(bookingId: string, status: string): Promise<Booking> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ensureMockLayerReady();
      await mockDelay();
      updateMockBooking(Number(bookingId), { status: status as Booking['status'] });
      const bookings = getMockBookings();
      return bookings.find(b => b.booking_id === Number(bookingId)) || {} as Booking;
    }
  ...
  async updateRoomPrice(typeId: string, newPrice: number): Promise<RoomType> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      for (const hotelId in allRoomTypes) {
        const rooms = allRoomTypes[hotelId];
        const index = rooms.findIndex(r => String(r.type_id) === String(typeId));
        if (index !== -1) {
...
export const paymentApi = {
  ...
  async createPayment(data: {
    ...
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      updateMockBooking(data.bookingId, { paymentStatus: 'pending', paymentMethod: data.paymentMethod });
      
      return {
        payment_id: paymentId,
        payment_url: data.paymentMethod === 'vnpay' ? `https://sandbox.vnpayment.vn/mock?ref=${paymentId}` : undefined,
  ...
  async getById(paymentId: string): Promise<Payment | null> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      const payments = JSON.parse(localStorage.getItem('payments') || '[]');
      return payments.find((p: Payment) => p.payment_id === Number(paymentId)) || null;
    }
  ...
  async getByBooking(bookingId: string): Promise<Payment | null> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      const payments = JSON.parse(localStorage.getItem('payments') || '[]');
      return payments.find((p: Payment) => p.booking_id === Number(bookingId)) || null;
    }
  ...
  async queryStatus(paymentId: string): Promise<{ status: string; vnp_response_code?: string }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      const payments = JSON.parse(localStorage.getItem('payments') || '[]');
      const payment = payments.find((p: Payment) => p.payment_id === Number(paymentId));
  ...
  async refund(paymentId: string, amount?: number): Promise<{ success: boolean; refund_id?: string }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      const payments = JSON.parse(localStorage.getItem('payments') || '[]');
      const index = payments.findIndex((p: Payment) => p.payment_id === Number(paymentId));
      if (index !== -1) {
        payments[index].status = 'refunded';
        payments[index].updated_at = new Date().toISOString();
        localStorage.setItem('payments', JSON.stringify(payments));
        // Update booking payment status
        updateMockBooking(payments[index].booking_id, { paymentStatus: 'refunded' });
      }
  ...
  async completePayment(paymentId: string): Promise<Payment> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      if (index !== -1) {
        ...
        localStorage.setItem('payments', JSON.stringify(payments));
        // Update booking payment status
        updateMockBooking(payments[index].booking_id, { paymentStatus: 'paid' });
...
export const roomInventoryApi = {
  async checkAvailability(data: {
    ...
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      return { available: true, availableCount: Math.floor(Math.random() * 5) + 1 };
    }
  ...
  async getRoomType(typeId: string): Promise<RoomType | null> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      const allRoomTypes = getMockRoomTypes();
      for (const hotelId in allRoomTypes) {
        const room = allRoomTypes[hotelId].find(r => String(r.type_id) === String(typeId));
        if (room) return room;
      }
      return null;
    }
  ...
  async getRoomsByType(typeId: string): Promise<Room[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      return [
        { room_id: 101, type_id: Number(typeId), name: 'Room 101', location: 'Floor 1', status: 1, estimated_available_time: null, number_of_single_beds: 1, number_of_double_beds: 1, room_view: 'City View', room_size: 30, notes: null },
        { room_id: 102, type_id: Number(typeId), name: 'Room 102', location: 'Floor 1', status: 1, estimated_available_time: null, number_of_single_beds: 2, number_of_double_beds: 0, room_view: 'Garden View', room_size: 28, notes: null },
      ];
    }
  ...
  async createHold(data: {
    ...
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      const expiresAt = new Date(Date.now() + (data.holdDurationMinutes || 15) * 60000).toISOString();
...
export const destinationsApi = {
  ...
  async getByType(type: string): Promise<Destination[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      await mockDelay();
      const spots = getMockTourismSpots();
      return spots.filter(s => s.type.toLowerCase() === type.toLowerCase());
    }
  ...
  async update(id: string, data: Partial<Destination>): Promise<Destination> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      await mockDelay();
      const spots = getMockTourismSpots();
      const index = spots.findIndex(s => s.destination_id === Number(id));
      if (index !== -1) {
    ...
    return apiClient.put<Destination>(API_CONFIG.ENDPOINTS.UPDATE_DESTINATION, { id, ...data });
  },
  ...
  async delete(id: string): Promise<boolean> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      await mockDelay();
      const spots = getMockTourismSpots();
      const filtered = spots.filter(s => s.destination_id !== Number(id));
      localStorage.setItem('tourismSpots', JSON.stringify(filtered));
      return true;
    }
...
export const notificationApi = {
  ...
  async sendBookingConfirmation(bookingId: number, email: string): Promise<{ success: boolean }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      console.log('Mock: Sending booking confirmation for', bookingId, 'to', email);
      return { success: true };
    }
  ...
  async sendBookingCancellation(bookingId: number, email: string): Promise<{ success: boolean }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      console.log('Mock: Sending booking cancellation for', bookingId, 'to', email);
      return { success: true };
    }
  ...
  async sendPasswordReset(email: string, resetUrl: string): Promise<{ success: boolean }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      console.log('Mock: Sending password reset to', email, 'with URL', resetUrl);
      return { success: true };
    }
  ...
  async sendPaymentConfirmation(paymentId: number, email: string): Promise<{ success: boolean }> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      console.log('Mock: Sending payment confirmation for', paymentId, 'to', email);
      return { success: true };
    }
...
export const adminApi = {
  ...
  async getDashboard(): Promise<AdminDashboard> {
    if (API_CONFIG.USE_MOCK_DATA) {
      ...
      return {
        ...
        activeBookings: 127,
        recentActivity: [
          { id: 1, type: 'booking', description: 'Đặt phòng mới tại Vinpearl Resort', timestamp: new Date().toISOString(), userName: 'Nguyễn Văn A' },
          { id: 2, type: 'hotel', description: 'Khách sạn mới đăng ký chờ duyệt', timestamp: new Date(Date.now() - 3600000).toISOString(), userName: 'Hotel Manager B' },
          { id: 3, type: 'user', description: 'Người dùng mới đăng ký', timestamp: new Date(Date.now() - 7200000).toISOString(), userName: 'Trần Thị C' },
          { id: 4, type: 'payment', description: 'Thanh toán thành công 2,500,000₫', timestamp: new Date(Date.now() - 10800000).toISOString(), userName: 'Lê Văn D' },
          { id: 5, type: 'review', description: 'Đánh giá mới 5 sao', timestamp: new Date(Date.now() - 14400000).toISOString(), userName: 'Phạm Thị E' },
        ],
      };
    }
  ...
  async getRecentActivity(): Promise<AdminActivity[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return [
        { id: 1, type: 'booking', description: 'Đặt phòng mới', timestamp: new Date().toISOString() },
        { id: 2, type: 'hotel', description: 'Khách sạn mới chờ duyệt', timestamp: new Date(Date.now() - 3600000).toISOString() },
      ];
    }
  ...
  async getAllUsers(): Promise<AdminUser[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return [
        { user_id: 1, name: 'Nguyễn Văn Admin', email: 'admin@hotel.com', role: 'admin', status: 'active', created_at: '2024-01-01', bookings_count: 0 },
        { user_id: 2, name: 'Trần Thị Manager', email: 'manager@hotel.com', phone: '0901234567', role: 'hotel_manager', status: 'active', created_at: '2024-02-15', bookings_count: 45 },
        { user_id: 3, name: 'Lê Văn Customer', email: 'customer@gmail.com', phone: '0912345678', role: 'customer', status: 'active', created_at: '2024-03-20', bookings_count: 12 },
        { user_id: 4, name: 'Phạm Thị Inactive', email: 'inactive@gmail.com', role: 'customer', status: 'inactive', created_at: '2024-04-10', bookings_count: 3 },
        { user_id: 5, name: 'Hoàng Văn Banned', email: 'banned@gmail.com', role: 'customer', status: 'banned', created_at: '2024-05-05', bookings_count: 1 },
      ];
    }
  ...
  async getUserById(id: string): Promise<AdminUser> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { user_id: parseInt(id), name: 'Mock User', email: 'mock@email.com', role: 'customer', status: 'active', created_at: '2024-01-01' };
    }
  ...
  async updateUserRole(id: string, role: AdminUser['role']): Promise<AdminUser> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { user_id: parseInt(id), name: 'Updated User', email: 'mock@email.com', role, status: 'active', created_at: '2024-01-01' };
    }
  ...
  async updateUser(id: string, data: Partial<AdminUser>): Promise<AdminUser> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { user_id: parseInt(id), name: data.name || 'Updated', email: data.email || 'mock@email.com', role: data.role || 'customer', status: data.status || 'active', created_at: '2024-01-01' };
    }
  ...
  async getHotelManagers(): Promise<AdminUser[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return [
        { user_id: 2, name: 'Trần Thị Manager', email: 'manager@hotel.com', phone: '0901234567', role: 'hotel_manager', status: 'active', created_at: '2024-02-15' },
        { user_id: 6, name: 'Nguyễn Văn HM', email: 'hm@hotel.com', role: 'hotel_manager', status: 'active', created_at: '2024-03-01' },
      ];
    }
  ...
  async getPendingHotels(): Promise<AdminHotel[]> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return [
        { hotel_id: 10, name: 'Khách sạn ABC', city: 'Hà Nội', district: 'Hoàn Kiếm', status: 'pending', manager_name: 'Nguyễn Văn A', manager_email: 'a@hotel.com', rooms_count: 25, created_at: '2024-12-20' },
        { hotel_id: 11, name: 'Resort XYZ', city: 'Đà Nẵng', district: 'Sơn Trà', status: 'pending', manager_name: 'Trần Thị B', manager_email: 'b@hotel.com', rooms_count: 50, created_at: '2024-12-22' },
      ];
    }
  ...
  async approveHotel(id: string): Promise<AdminHotel> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { hotel_id: parseInt(id), name: 'Approved Hotel', city: 'HN', district: 'HK', status: 'approved', manager_name: 'M', manager_email: 'm@h.com', rooms_count: 10, created_at: '2024-01-01' };
    }
  ...
  async lockHotel(id: string): Promise<AdminHotel> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { hotel_id: parseInt(id), name: 'Locked Hotel', city: 'HN', district: 'HK', status: 'locked', manager_name: 'M', manager_email: 'm@h.com', rooms_count: 10, created_at: '2024-01-01' };
    }
  ...
  async updateHotelStatus(id: string, status: AdminHotel['status']): Promise<AdminHotel> {
    if (API_CONFIG.USE_MOCK_DATA) {
      await mockDelay();
      return { hotel_id: parseInt(id), name: 'Hotel', city: 'HN', district: 'HK', status, manager_name: 'M', manager_email: 'm@h.com', rooms_count: 10, created_at: '2024-01-01' };
    }
